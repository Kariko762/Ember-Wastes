    ...
    while player_data["hull"] > 0 and enemy_npc["hull"] > 0:
        ...
        for attacker, attacker_type in turn_order:
            ...
            if attacker_type == "Player":
                ...
                sys.stdout.flush()
                action = input().strip()
                full_line = TextStyle.print_class("Information", f"Action: (1) Attack, (2) Disengage: {action}", delay_to_display=0, display_mode="instant", print_output=False)
        ...
        TextStyle.print_class("Information", "- - -")  # Separator between rounds
